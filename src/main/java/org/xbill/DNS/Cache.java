...
public class Cache {
    ...
    private static class CacheMap extends LinkedHashMap<Object, Object> {
        /**
     * 
     */
    ...
    private static class CacheRRset extends RRset implements Element {
        ...
        public CacheRRset(Record rec, int cred, long maxttl) {
            super();
            credibility = cred;
            expire = limitExpire(rec.getTTL(), maxttl);
            addRR(rec);
        }
    ...
    public SetResponse addMessage(Message in) {
        ...
        for (RRset element : auth) {
            if (element.getType() == Type.SOA
                && curname.subdomain(element.getName())) {
                soa = element;
            } else if (element.getType() == Type.NS
                       && curname.subdomain(element.getName())) {
                ns = element;
            }
        }
